...
// 【重要】必须同步导入！useMessage/useDialog 等 naive-ui hook 需要在组件 setup 内调用。
// 异步加载的组件 setup 运行于 app.mount() 返回之后，此时 getCurrentInstance() 返回 null，
// 导致 naive-ui "No outer <n-message-provider />" 错误。
import PlayBar from '@/components/player/PlayBar.vue';
import MobilePlayBar from '@/components/player/MobilePlayBar.vue';
import PlayingListDrawer from '@/components/player/PlayingListDrawer.vue';
