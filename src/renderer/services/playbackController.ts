...
// 懒初始化 message API，避免 createDiscreteApi 在模块求值阶段触发循环依赖
// （当 PlayBar 同步导入时，playbackController 会在主 chunk 初始化期间被求值，
//  此时 naive-ui 内部的部分 lexical declaration 尚未完成初始化）
  ...
  audioService.on('url_expired', async (expiredTrack: SongResult) => {
    ...
    try {
      ...
      if (success) {
        ...
        getMessage().success(i18n.global.t('player.autoResumed'));
      } else {
        // 检查歌曲是否仍然是当前歌曲
        const currentPlayerCore = await getPlayerCoreStore();
        if (currentPlayerCore.playMusic?.id === expiredTrack.id) {
          getMessage().error(i18n.global.t('player.resumeFailed'));
        }
      }
    } catch (error) {
